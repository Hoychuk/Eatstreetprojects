...
## Product essence
EatStreet is an American online food ordering marketplace connecting diners with local restaurants for delivery and takeout. Currently, EatStreet claims partnerships with approximately 15,000+ restaurants across 150+ U.S. markets, with delivery fulfilled via contracted third-party providers (e.g., UberEats, DoorDash drivers) since August 2023.

Key value propositions:

**For diners:** Simple, centralized ordering from local restaurants with real-time tracking, group ordering, scheduled ordering, referral rewards, and exclusive deals.  
**For restaurants:** A marketplace listing + tools for custom websites, branded menu pages, POS integration, digital marketing, and loyalty programs — with no upfront cost, commission-based pricing.

---
## Subsystems (observable)

From a QA team perspective, EatStreet should be analyzed as a **black-box, user-facing e-commerce and food delivery platform**, with testing efforts focused on observable user interactions and end-to-end flows.

The primary QA focus areas include:

- **Location-based discovery** — address input, validation, and restaurant availability;
- **Restaurant listing and filtering** — search results, sorting, availability states, and cuisine filters;
- **Menu browsing and item configuration** — categories, item details, and modifiers;
- **Cart behavior** — item management, quantity updates, price recalculation, and group ordering;
- **Checkout flow and validation** — required fields, delivery/pickup options, scheduled ordering, coupon, and order summary;
- **Authentication and account flows** — sign in, sign up, password recovery, and user data management;
- **Order tracking** — real-time status updates and post-order flows;
- **Promotions and rewards** — coupons, referral program, newsletter sign-up discounts;
- **Support and recovery flows** — help center, FAQ, contact forms, and post-order assistance;
- **Mobile apps** — iOS and Android parity with web flows.
...
- Some flows may behave differently for guest and registered users.
- Pricing may change based on quantity, modifiers, delivery fees, taxes, or service fees.
- Scheduled ordering availability depends on the restaurant.
- Referral and promo codes have minimum order thresholds and single-use restrictions.
- Restaurant Dashboard and internal merchant tools are not publicly testable.
- Real payment and order fulfillment should not be tested in a study project.
